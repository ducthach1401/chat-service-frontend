...
const MenuLeft = memo((props: MenuLeftProps) => {
  ...
  const checkUrl = (url: any, route: any) => {
    if (route.subMenu) {
      return route.subMenu.some((el: any) => {
        if (el.subsubMenu)
          return el.subsubMenu.some((elSub: any) => elSub.path === url);
  ...
  useEffect(() => {
    ...
    if (active) {
      ...
      setCollapseSubMenu(true);
      const [subActive] = active.subMenu
        ? active.subMenu.filter((subRoute) => checkUrl(url, subRoute))
        : [];
      if (subActive) {
    ...
    <div className={classes.root}>
      ...
      >
        <div className={classes.toolbarMenu}>
          <img alt="ic-logo" className="ic-logo" src={Images.logo} />
          {(open || openBtn) && (
            <img alt="logo" className="logo" src={Images.logoRec} />
          )}
          {(open || openBtn) && (
        ...
        <Grid className={classes.list}>
          <List
            classes={{
              root: open || openBtn ? classes.listOpen : classes.listClose,
            }}
          >
              ...
              {getMenus().map((route, index) => (
                <Fragment key={`routes-${index}`}>
                  <ListItem
                    ...
                    {...{}}
                    component={
                      route.subMenu && route.subMenu.length > 0 ? Button : Link
                    }
                    to={route.path}
                        ...
                        route.subMenu.map((el, i) => {
                          return (
                            <Fragment key={`router-${index}-${i}`}>
                              ...
                              >
                                <ListItemIcon className="iconList subTitle">
                                  {el.icon}
                                </ListItemIcon>
                                <ListItemText className="textList">
                                  {el.subTitle}
                                    ...
                                    el.subsubMenu.map((elSub, ii) => {
                                      return (
                                        <ListItem
                                          ...
                                          classes={{
                                            selected: classes.selectedClose,
                                          }}
                                        >
